...
export function UnifiedTimeline({
  ...
  return (
    <LiquidGlass className="mb-6 rounded-2xl border border-white/15 bg-[#0B1433]">
      <div className="flex flex-col gap-4 p-5 sm:p-6">
        ...
        <ol className="grid gap-2 lg:grid-cols-2">
          {items.slice(0, 16).map((item) => (
            ...
            >
              <span
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-mono text-[9px] font-semibold"
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className={`truncate font-mono text-xs ${item.done ? 'line-through' : ''}`}>
                    {item.title}
                  </span>
                  {item.duplicateCount > 1 && (
                    <span
                      title={`중복 출처: ${item.duplicateProviders.map(providerMark).join(', ')}`}
                      className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-300/10 px-1.5 py-0.5 font-mono text-[9px] text-amber-200"
                    >
                      <Link2 size={9} />
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2 font-mono text-[9px] text-cream/45">
              ...
              {item.externalUrl && (
                <a
                  ...
                  title="원본 앱에서 열기"
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/15 text-cream/55 transition hover:border-neon/50 hover:text-neon"
                >
          ...
          {items.length === 0 && (
            <li className="font-mono text-xs text-cream/40">
