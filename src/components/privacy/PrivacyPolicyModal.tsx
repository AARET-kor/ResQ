      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[24px] bg-[#EFF5EC] p-6 text-[#1c3325] shadow-2xl sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="font-sans text-sm text-[#1c3325]/60">정책 버전 2026-07-24</p>
          </div>
        ...
        <div className="mt-5 space-y-4 font-sans text-xs leading-relaxed">
          <section>
        ...
        <section className="mt-6">
          <div className="flex items-center justify-between gap-3">
            <h4 className="font-sans text-xs font-bold uppercase">최근 처리 기록</h4>
            <button type="button" onClick={onDeleteAudits} disabled={deleting || audits.length === 0}
              className="rounded border border-[#1c3325]/30 px-3 py-1 font-sans text-sm disabled:opacity-40">
              {deleting ? '삭제 중…' : '기록 삭제'}
            </button>
          </div>
          <ul className="mt-2 flex flex-col gap-2">
            {audits.map((audit) => (
              <li key={audit.id} className="rounded-lg bg-[#1c3325]/5 px-3 py-2 font-sans text-sm">
            ...
            {audits.length === 0 && (
              <li className="font-sans text-sm text-[#1c3325]/60">저장된 처리 기록이 없습니다.</li>
            )}
          </ul>
        </section>
      </div>
