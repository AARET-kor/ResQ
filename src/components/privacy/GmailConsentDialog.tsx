...
export function GmailConsentDialog({
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/70 p-4 sm:items-center"
      ...
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-[24px] border border-white/15 bg-[#0B1433] p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          ...
          <button aria-label="동의 창 닫기" onClick={onClose}
            className="shrink-0 text-cream/60 transition hover:text-cream">
            <X size={18} />
          </button>
        </div>

        <div className="mt-5 flex flex-col gap-3">
          <label className="flex items-start gap-3 rounded-xl bg-white/5 p-3 font-mono text-xs leading-relaxed">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
          <button type="button" onClick={onShowPolicy}
            className="font-mono text-[11px] text-cream/70 underline">
            개인정보 처리·삭제 정책 보기
          </button>
          <button type="button" disabled={!ready || busy}
            onClick={async () => { if (await onConfirm()) onClose() }}
            className="rounded-md bg-neon px-4 py-2 font-grotesk text-xs uppercase text-bg disabled:cursor-not-allowed disabled:opacity-40">
            {busy ? '저장 중…' : '동의하고 사용'}
          </button>
        </div>
      </div>
