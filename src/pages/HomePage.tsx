...
export function HomePage({
  ...
  return (
    <>
      <div className="sticky top-0 z-40 border-b border-white/10 bg-[#020817]/88 px-4 py-3 backdrop-blur-xl">
          ...
          <ul className="flex min-w-max items-center justify-center gap-2">
            {(['plan', 'team', 'papers', 'integrations'] as const).map((route) => (
              <li key={route}>
                <AppLink
                  to={route}
                  className="block rounded-full border border-white/15 px-4 py-2 font-mono text-[10px] text-cream/60 transition hover:border-neon/50 hover:text-neon"
                >
                  {ROUTE_LABEL[route]}
                </AppLink>
              </li>
      ...
      <div className="mx-auto flex max-w-[1500px] flex-col gap-28 px-5 py-20 sm:px-8 sm:py-28">
        <section aria-labelledby="briefing-title">
          <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <p className="max-w-md font-mono text-xs leading-relaxed text-cream/45">
              홈에서는 상태만 빠르게 읽고, 실제 입력과 관리는 각 기능 페이지에서 집중해서 처리합니다.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {stats.map(({ label, value, detail, Icon }) => (
              <LiquidGlass key={label} className="rounded-2xl bg-white/[0.035]">
                <div className="flex min-h-40 flex-col justify-between p-5">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[10px] uppercase text-cream/45">{label}</span>
                    <Icon size={16} className="text-neon" />
                  </div>
                  <div>
                    <strong className="font-grotesk text-5xl font-normal text-cream">{value}</strong>
                    <p className="mt-2 truncate font-mono text-[10px] text-cream/40">{detail}</p>
                  </div>
                </div>
              </LiquidGlass>
            ))}
        ...
        <section aria-labelledby="timeline-title">
          <div className="mb-7 flex items-end justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#69C7FF]">
                Unified flow
              </p>
            ...
            <AppLink
              to="plan"
              className="hidden items-center gap-2 font-mono text-[10px] uppercase text-cream/50 transition hover:text-neon sm:flex"
            >
        ...
        <section aria-labelledby="workspace-title">
          <div className="mb-10 max-w-3xl">
            <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#FFB86B]">
              Focused workspaces
            </p>
            <h2 id="workspace-title" className="mt-2 font-grotesk text-4xl uppercase sm:text-6xl">
              기능마다<br />자기만의 공간
            </h2>
            <p className="mt-5 font-mono text-xs leading-relaxed text-cream/50">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {FEATURE_CARDS.map(({ route, number, title, description, Icon, accent }) => (
              <AppLink
                key={route}
                to={route}
                className="group relative min-h-72 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.035] p-6 transition hover:-translate-y-1 hover:border-white/25 sm:p-8"
              >
                <div
                  aria-hidden
                ...
                <div className="relative flex h-full flex-col justify-between">
                  <div className="flex items-start justify-between">
                    <span className="font-mono text-xs" style={{ color: accent }}>{number}</span>
                    <Icon size={27} style={{ color: accent }} />
                  </div>
                  <div>
                    <h3 className="font-grotesk text-3xl uppercase sm:text-4xl">{title}</h3>
                    <p className="mt-3 max-w-lg font-mono text-xs leading-relaxed text-cream/45">
                      {description}
                    </p>
                    <span className="mt-6 inline-flex items-center gap-2 font-mono text-[10px] uppercase text-cream/55 transition group-hover:text-cream">
        ...
        <section className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]" aria-labelledby="research-home-title">
          <div className="rounded-3xl border border-white/10 bg-[#111735] p-7 sm:p-10">
            <div className="flex items-center gap-2 text-[#FFB86B]">
              <Microscope size={18} />
              <span className="font-mono text-[10px] uppercase tracking-[0.2em]">Research pulse</span>
            </div>
            <h2 id="research-home-title" className="mt-8 font-grotesk text-4xl uppercase sm:text-6xl">
              {profile.specialty}<br />근거를 놓치지 않게
            </h2>
            <p className="mt-5 max-w-xl font-mono text-xs leading-relaxed text-cream/50">
              핵심 의학 저널과 전공 학회지를 함께 탐색하고, 선택한 논문을 분석 보고서와 연구 아이디어로 발전시킵니다.
            </p>
            <AppLink
              to="papers"
              className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#FFB86B] px-5 py-3 font-grotesk text-xs uppercase text-bg transition hover:opacity-90"
            >
              논문 워크스페이스 <ArrowRight size={14} />
            </AppLink>
          </div>
          <div className="flex flex-col gap-3 rounded-3xl border border-white/10 bg-white/[0.035] p-7 sm:p-10">
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] uppercase text-cream/45">Curated sources</span>
              <BookOpenCheck size={16} className="text-[#FFB86B]" />
            </div>
            <div className="mt-4 flex flex-1 flex-col justify-center gap-3">
              {journals.map((journal, index) => (
                <div key={journal.id} className="flex items-center gap-4 border-b border-white/10 pb-3">
                  <span className="font-mono text-[10px] text-cream/25">
                    {String(index + 1).padStart(2, '0')}
                  </span>
                  <span className="min-w-0 flex-1 truncate font-mono text-xs text-cream/70">
                    {journal.label}
                  </span>
                  {journal.oa && (
                    <span className="rounded-full bg-neon/10 px-2 py-1 font-mono text-[8px] text-neon">OA</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <section className="rounded-[32px] border border-white/10 bg-gradient-to-br from-[#111735] to-[#071126] p-7 sm:p-12">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            <div>
              <div className="flex items-center gap-2 text-[#B9A2FF]">
                <Activity size={17} />
                <span className="font-mono text-[10px] uppercase tracking-[0.2em]">Connected by design</span>
              </div>
            ...
            <div>
              <p className="font-mono text-xs leading-relaxed text-cream/50">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {['Google', 'Outlook', 'Todoist', 'Apple', 'Galaxy', 'ICS', 'CalDAV'].map((provider) => (
                  <span key={provider} className="rounded-full border border-white/15 px-3 py-2 font-mono text-[9px] text-cream/55">
                    {provider}
                  </span>
                ))}
              </div>
              <div className="mt-7 flex flex-wrap gap-4">
                <AppLink
                  to="integrations"
                  className="inline-flex items-center gap-2 rounded-full bg-[#B9A2FF] px-5 py-3 font-grotesk text-xs uppercase text-bg"
                >
                  연동 관리 <ArrowRight size={14} />
                </AppLink>
                <div className="flex items-center gap-2 font-mono text-[10px] text-cream/40">
        ...
        <section className="py-10 text-center">
          ...
          <p className="mt-6 font-mono text-[10px] uppercase tracking-[0.28em] text-cream/35">
          ...
          <p className="mx-auto mt-6 max-w-xl font-mono text-xs leading-relaxed text-cream/45">
            반복되는 일정 정리, 팀 조율, 논문 탐색을 덜어내고 중요한 판단에 더 집중할 수 있도록 설계했습니다.
          </p>
          <AppLink
            to="plan"
            className="mt-9 inline-flex items-center gap-2 rounded-full border border-neon/60 px-6 py-3 font-grotesk text-xs uppercase text-neon transition hover:bg-neon hover:text-bg"
          >
